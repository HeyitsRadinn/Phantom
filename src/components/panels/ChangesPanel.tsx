  ...
  return (
    <ul className="space-y-1 text-sm">
      {files.map((file) => (
        <li
          key={file.filepath}
          className="flex items-center justify-between px-2 py-1 rounded hover:bg-zinc-100 dark:hover:bg-zinc-800"
        >
...
const ChangesPanel: React.FC = () => {
  ...
  // determine content based on repo path and loading/error states
  let content;
  if (!activeRepoPath) {
    content = (
      <p className="text-sm text-zinc-500 dark:text-zinc-400 text-center p-4">
        Open a repository to view changes.
      </p>
    );
    ...
    content = <FileStatusList files={fileStatus} />;
  }
